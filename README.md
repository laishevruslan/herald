...
- **Meeting-room signs** — point a slide at a room's calendar to show Busy / Available, what's on now, and when the room frees up — in every dashboard language, defaulting to English. After the last meeting of the day the resolver supplies a localised empty-board hint (`remaining_today_empty`) instead of leaving a `Next:  ()` line
