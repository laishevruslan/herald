...
- **Meeting-room signs** — Slides → New deck → Meeting Room Door Sign, pick the room calendar, and publish. E-paper (800×480 Sticky, black/white) and TV 16:9 (green/red bar) share the same iCal source. After the last meeting of the day the resolver supplies a localised empty-board hint (`remaining_today_empty`) instead of leaving a `Next:  ()` line
- **Waste collection reminders** — Slides → New deck → Waste Collection Reminder. E-paper 800×480 (black/white, next bag huge) or kitchen TV 16:9 (week list). Bind a municipal ICS and optionally an Include filter (`Gelber Sack` / `Restmüll` / `Papier`); the following pickup hides when the calendar has only one event left
- **Daily office agenda** — Slides → New deck → Daily Office Agenda. A 16:9 lobby board (same JSON on 4K) with a live date, eight timed rows, and a localised “no more meetings today” line after the last event. Not for 1-bit e-paper.
