...
- **Waste factory `waste-epaper-5x3`:** New Deck → Waste Collection Reminder (E-Paper). Same 5:3 / 1-bit rules, no traffic-light fills — fraction identity is the calendar word (`Gelber Sack`). Do not use `waste-lcd-16x9` on 1-bit panels: muted grey body text dithers. See [`slide-data-binding.md`](slide-data-binding.md).
