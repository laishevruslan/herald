...
- **Data-source slides:** A slide that binds `{{ds:slug.field}}` is rendered with the **same** workspace map the widget player uses (`dataResolverFor` → `getWorkspaceDataMapSync`). That map stamps each source's `last_status` as reserved `__status`, so `hide_if_empty`, `show_when`, `bind_status`, and `color_when` apply on the embedded path exactly as they do on the wall. A failed calendar fetch paints **stale**, not Available.
- **Meeting-room factory `room-epaper-5x3`:** New Deck → Meeting Room Door Sign (E-Paper). Aspect `5:3` (800×480), colours only `#000` / `#FFF`, motion none — the geometry that survives Floyd–Steinberg on a Seeed reTerminal Sticky (`seeed-reterminal-sticky`) or Waveshare 7.5″. Busy inverts the status word (white on a black box); free is black on white; stale is a half-width black rule. Publish the deck to the device's playlist. Do not use `room-lcd-16x9`, `room-lcd-9x16`, or `rooms-board-16x9` on 1-bit panels: traffic-light bars dither to noise.
- **Waste factory `waste-epaper-5x3`:** New Deck → Waste Collection Reminder (E-Paper). Same 5:3 / 1-bit rules, no traffic-light fills — fraction identity is the calendar word (`Gelber Sack`). Do not use `waste-lcd-16x9` or `agenda-lcd-16x9` on 1-bit panels: muted grey body text dithers. See [`slide-data-binding.md`](slide-data-binding.md).
