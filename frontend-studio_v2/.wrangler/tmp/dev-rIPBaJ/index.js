...
__name(openApiDocFn, "openApiDocFn");
var OWN_PATHS = /* @__PURE__ */ new Set(["/api/openapi.json", "/llms.txt"]);
function docConfig(opts) {
  return {
    openapi: "3.1.0",
    info: {
      title: opts.title ?? "App API",
      version: opts.version ?? "1.0.0",
      ...opts.description ? { description: opts.description } : {}
    }
  };
}
__name(docConfig, "docConfig");
function toOpenApiPath(path) {
  return path.replace(/:([A-Za-z0-9_]+)/g, "{$1}");
}
__name(toOpenApiPath, "toOpenApiPath");
function apiRoutes(app2) {
  const seen = /* @__PURE__ */ new Set();
  const out = [];
  const rows = app2.routes ?? [];
  for (const r of rows) {
    if (!r || typeof r.path !== "string" || typeof r.method !== "string") continue;
    if (r.method === "ALL") continue;
    if (!r.path.startsWith("/api")) continue;
    if (OWN_PATHS.has(r.path)) continue;
...
function buildDoc(app2, opts) {
  const genDoc = openApiDocFn(app2);
  if (genDoc) {
    try {
      return genDoc(docConfig(opts));
    } catch {
    }
  }
  const paths = {};
  for (const r of apiRoutes(app2)) {
    const p = toOpenApiPath(r.path);
    (paths[p] ??= {})[r.method.toLowerCase()] = {
      responses: { "200": { description: "OK" } }
    };
  }
  return { ...docConfig(opts), paths };
}
__name(buildDoc, "buildDoc");
function endpointEntries(app2, doc) {
  const out = [];
  for (const [path, ops] of Object.entries(doc.paths ?? {})) {
    for (const [method, op] of Object.entries(ops)) {
      const o = op;
      const note = o?.summary || o?.description;
      out.push({ method: method.toUpperCase(), path, ...note ? { summary: note } : {} });
    }
  }
  const documented = documentedSet(doc);
  for (const r of apiRoutes(app2)) {
    if (!documented.has(`${r.method} ${toOpenApiPath(r.path)}`)) {
      out.push({ method: r.method, path: r.path });
    }
  }
  return out;
}
__name(endpointEntries, "endpointEntries");
function documentedSet(doc) {
  const s = /* @__PURE__ */ new Set();
  for (const [p, ops] of Object.entries(doc.paths ?? {})) {
    for (const m of Object.keys(ops)) s.add(`${m.toUpperCase()} ${p}`);
  }
  return s;
}
__name(documentedSet, "documentedSet");
function buildLlmsIndex(app2, opts) {
  const doc = buildDoc(app2, opts);
  const info3 = doc.info ?? {};
  const title2 = info3.title || opts.title || "App API";
  const lines = [`# ${title2}`];
  const summary = info3.description || opts.description;
  if (summary) lines.push("", `> ${summary}`);
  const entries = endpointEntries(app2, doc).map(
    (e) => `- \`${e.method} ${e.path}\`${e.summary ? ` \u2014 ${e.summary}` : ""}`
  );
  if (entries.length) lines.push("", "## API", ...entries);
  lines.push("", "Full machine-readable spec: [/api/openapi.json](/api/openapi.json)");
  return lines.join("\n") + "\n";
}
__name(buildLlmsIndex, "buildLlmsIndex");
function mountDiscovery(app2, opts = {}) {
  app2.get("/api/openapi.json", (c) => c.json(buildDoc(app2, opts)));
  app2.get(
    "/llms.txt",
    (c) => c.text(buildLlmsIndex(app2, opts), 200, {
      "Content-Type": "text/markdown; charset=utf-8",
      "Cache-Control": "public, max-age=300"
    })
  );
  app2.notFound((c) => {
    if (!c.req.path.startsWith("/api")) return c.text("404 Not Found", 404);
    const body = {
      error: "not_found",
      message: `No route for ${c.req.method} ${c.req.path} on this app.`,
      hint: "GET /llms.txt lists every endpoint; GET /api/openapi.json has full request/response schemas."
    };
    const caller = c.req.header("X-Clawnify-Caller");
    if (caller && caller !== "public") {
      body.endpoints = endpointEntries(app2, buildDoc(app2, opts));
    }
    return c.json(body, 404);
  });
}
__name(mountDiscovery, "mountDiscovery");

// node_modules/.pnpm/@clawnify+db@0.4.2/node_modules/@clawnify/db/dist/index.js
var _impl = null;
function initDB(arg) {
...
async function query(sql2, params = []) {
  ...
  return ensure().query(sql2, params);
}
__name(query, "query");
async function get(sql2, params = []) {
  assertBindable(params);
  return ensure().get(sql2, params);
}
__name(get, "get");
async function run(sql2, params = []) {
  assertBindable(params);
  return ensure().run(sql2, params);
}
__name(run, "run");
function assertBindable(params) {
  for (let i = 0; i < params.length; i++) {
    ...
    throw new TypeError(
      `@clawnify/db: param [${i}] is ${t} \u2014 D1/SQLite only binds scalars. If this is a JSON column, wrap with json(value). Got: ${preview}`
    );
  }
}
__name(assertBindable, "assertBindable");
function d1Impl(db) {
  return {
    async query(sql2, params) {
      const { results } = await db.prepare(sql2).bind(...params).all();
      return results ?? [];
    },
    async get(sql2, params) {
      const row = await db.prepare(sql2).bind(...params).first();
      return row ?? void 0;
    },
    async run(sql2, params) {
      const r = await db.prepare(sql2).bind(...params).run();
      return {
...
function storageImpl(s) {
  return {
    async query(sql2, params) {
      const result = await s.query(sql2, params);
      return result.rows ?? [];
    },
    async get(sql2, params) {
      const result = await s.query(sql2, params);
...
// node_modules/.pnpm/@clawnify+app@0.2.1_@hono+zod-openapi@0.18.4_hono@4.12.10_zod@3.25.76__@phosphor-icons+react@2.1.10_hono@4.12.10/node_modules/@clawnify/app/dist/index.js
...
var _bucket;
function initUploads(bucket) {
  _bucket = bucket;
}
__name(initUploads, "initUploads");
function sanitize(filename) {
  return filename.replace(/[^a-zA-Z0-9._-]/g, "");
}
__name(sanitize, "sanitize");
async function putUpload(filename, data, contentType) {
...
__name(getUpload, "getUpload");

// src/server/seed-templates.ts
var SEED_TEMPLATES = [
  {
    id: "quote-card",
    name: "Quote Card",
...
// src/server/index.ts
var app = createApp({ title: "OpenDesign API", version: "1.0.0" });
var seeded = false;
async function ensureSeeded() {
  if (seeded) return;
  seeded = true;
  try {
    const existing = await get("SELECT COUNT(*) as c FROM templates");
...
var DesignSchema = external_exports.object({
  ...
  canvas_json: external_exports.string(),
  width: external_exports.number(),
  height: external_exports.number(),
  thumbnail_url: external_exports.string().nullable(),
...
var TemplateSchema = external_exports.object({
  ...
  height: external_exports.number(),
  thumbnail_url: external_exports.string().nullable(),
  sort_order: external_exports.number()
});
var PageSchema = external_exports.object({
  id: external_exports.string(),
  design_id: external_exports.string(),
  title: external_exports.string(),
  canvas_json: external_exports.string(),
  sort_order: external_exports.number(),
  created_at: external_exports.string()
});
var DesignWithPagesSchema = DesignSchema.extend({
  pages: external_exports.array(PageSchema)
});
var ErrorSchema = external_exports.object({ error: external_exports.string() });
var listDesigns = createRoute({
  method: "get",
  path: "/api/designs",
  responses: { 200: { content: { "application/json": { schema: external_exports.array(DesignSchema) } }, description: "OK" } }
});
app.openapi(listDesigns, async (c) => {
  const rows = await query("SELECT * FROM designs ORDER BY updated_at DESC");
  return c.json(rows, 200);
});
var getDesign = createRoute({
  method: "get",
  path: "/api/designs/{id}",
...
app.openapi(getDesign, async (c) => {
  ...
  const row = await get("SELECT * FROM designs WHERE id = ?", [id]);
      ...
      content: {
        "application/json": {
          schema: external_exports.object({
            ...
            canvas_json: external_exports.string().optional(),
      ...
      content: {
        "application/json": {
          schema: external_exports.object({
            ...
            width: external_exports.number().optional(),
            height: external_exports.number().optional(),
            thumbnail_url: external_exports.string().optional()
          })
        }
      }
    }
...
app.openapi(updateDesign, async (c) => {
  ...
  const existing = await get("SELECT * FROM designs WHERE id = ?", [id]);
...
var deleteDesign = createRoute({
  method: "delete",
  path: "/api/designs/{id}",
  request: { params: external_exports.object({ id: external_exports.string() }) },
  responses: {
    200: { content: { "application/json": { schema: external_exports.object({ ok: external_exports.boolean() }) } }, description: "OK" }
  }
});
      ...
      content: {
        "application/json": {
          schema: external_exports.object({
            ...
            canvas_json: external_exports.string().optional(),
            after_sort_order: external_exports.number().optional()
          })
        }
      }
    }
...
app.openapi(addPage, async (c) => {
  const { id } = c.req.valid("param");
  const body = c.req.valid("json");
  const count3 = await get("SELECT COUNT(*) as c FROM pages WHERE design_id = ?", [id]);
  const title2 = body.title || `Page ${(count3?.c ?? 0) + 1}`;
  let insertOrder;
  if (body.after_sort_order !== void 0) {
    await run(
    ...
    const maxOrder = await get("SELECT COALESCE(MAX(sort_order), -1) as m FROM pages WHERE design_id = ?", [id]);
    insertOrder = (maxOrder?.m ?? -1) + 1;
  }
  const page = await get(
    "INSERT INTO pages (design_id, title, canvas_json, sort_order) VALUES (?, ?, ?, ?) RETURNING *",
    [id, title2, body.canvas_json || "{}", insertOrder]
  );
  return c.json(page, 200);
});
var duplicatePage = createRoute({
  method: "post",
  path: "/api/pages/{pageId}/duplicate",
  request: { params: external_exports.object({ pageId: external_exports.string() }) },
  responses: {
...
app.openapi(duplicatePage, async (c) => {
  ...
  const original = await get("SELECT * FROM pages WHERE id = ?", [pageId]);
  ...
  const page = await get("SELECT * FROM pages WHERE design_id = ? AND sort_order = ?", [original.design_id, original.sort_order + 1]);
  return c.json(page, 200);
});
var updatePage = createRoute({
  method: "put",
  path: "/api/pages/{pageId}",
  request: {
    params: external_exports.object({ pageId: external_exports.string() }),
    body: {
      content: {
        "application/json": {
          schema: external_exports.object({
...
app.openapi(updatePage, async (c) => {
  const { pageId } = c.req.valid("param");
  const body = c.req.valid("json");
  const existing = await get("SELECT * FROM pages WHERE id = ?", [pageId]);
  if (!existing) return c.json({ error: "Not found" }, 404);
  await run(
...
app.openapi(deletePage, async (c) => {
  ...
  const count3 = await get("SELECT COUNT(*) as c FROM pages WHERE design_id = ?", [page.design_id]);
  if ((count3?.c ?? 0) <= 1) return c.json({ error: "Cannot delete the last page" }, 400);
  await run("DELETE FROM pages WHERE id = ?", [pageId]);
  return c.json({ ok: true }, 200);
});
var listTemplates = createRoute({
  method: "get",
  path: "/api/templates",
  responses: { 200: { content: { "application/json": { schema: external_exports.array(TemplateSchema) } }, description: "OK" } }
});
app.openapi(listTemplates, async (c) => {
  const rows = await query("SELECT * FROM templates ORDER BY sort_order");
  return c.json(rows, 200);
});
var getTemplate = createRoute({
  method: "get",
...
app.post("/api/uploads", async (c) => {
  ...
  const allowed = /* @__PURE__ */ new Set(["png", "jpg", "jpeg", "gif", "webp", "svg"]);
  if (!allowed.has(ext)) {
...
// .wrangler/tmp/bundle-hxAIdh/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-hxAIdh/middleware-loader.entry.ts
