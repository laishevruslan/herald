...
function resolveOf(payload) {
  return {
    resolveData: (slug, key) => (slug === 'room' ? payload[key] : undefined),
  };
}

test('listFactories names the two T1 ids and no waste/agenda yet', () => {
  const ids = listFactories().map((f) => f.id);
  assert.deepEqual(ids, ['room-epaper-5x3', 'room-lcd-16x9']);
});

test('unknown factory is null', () => {
  assert.equal(buildFactory('room-epaper-9x16'), null);
  assert.equal(buildDeck('waste-epaper-5x3', { slug: 'abfall' }), null);
});
...
test('factory bindings use only CANON keys — never next_event_title or organizer', () => {
  for (const id of ['room-epaper-5x3', 'room-lcd-16x9']) {
