...
// The throwaway one-item playlist a content-only schedule invents is auto-generated by
// construction (owns the is_auto_generated flag); its lifecycle helpers live here too.
const {
...
router.post('/', (req, res) => {
  ...
  if (!effectivePlaylistId && content_id) {
    ...
    const genId = createAutoGeneratedPlaylist(db, {
      name: `Scheduled: ${(c && c.filename) || 'item'}`,
      workspaceId: targetWorkspaceId, userId: req.user.id, status: 'published',
    });
...
router.put('/:id', requireScheduleWrite, (req, res) => {
  ...
  // The generated playlist holds a copy of the content, not a reference: a content change
  // must rewrite the single item or the screen keeps playing the old file while the
  // calendar shows the new one. Explicit overrides are left alone (content_id is
  // vestigial there); adopted multi-item playlists belong to their human.
  // A playlist pointer that moved away orphans the old throwaway: collect it.
  const contentChanged = req.body.content_id !== undefined
    && req.body.content_id && req.body.content_id !== schedule.content_id;
  const playlistChanged = req.body.playlist_id !== undefined
    && req.body.playlist_id !== schedule.playlist_id;
  if (contentChanged && !playlistChanged
    && rewriteScheduledPlaylistItem(db, schedule.playlist_id, req.body.content_id)) {
    // Publish through the shared path, same as POST: players read the snapshot.
    require('./playlists').publishPlaylist(schedule.playlist_id);
  }
  if (playlistChanged) deleteAutoGeneratedPlaylistIfOrphan(db, schedule.playlist_id);
...
router.delete('/:id', requireScheduleWrite, (req, res) => {
  const playlistId = req.schedule.playlist_id;
  db.prepare('DELETE FROM schedules WHERE id = ?').run(req.params.id);
  // The throwaway the POST handler invented has no other reason to exist. The helper
  // only removes untouched orphans (auto-generated, single item, unreferenced).
  deleteAutoGeneratedPlaylistIfOrphan(db, playlistId);
