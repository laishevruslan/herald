...
- **Plugin loader bypass** (an uploaded zip `require()`'d without platform-admin
  approval of that exact tree hash, the inbox treated as a plugin root, or a
  pinned plugin loading after its files changed)
...
- **A plugin that was explicitly approved, enabled, and loaded by the
  operator.** Plugins are trusted local code, same as `server/lib`. Approval
  is a human gate, not a sandbox. See `docs/plugins.md`.
